...
- `app.css` — Styles: 3 rules, 1 vars (~100 tok)
- `root.tsx` — links (~511 tok)
- `routes.ts` — Declares RouteConfig (~164 tok)

## app/lib/

- `auth.server.test.ts` — Declares cookieHeaderFor (~596 tok)
- `auth.server.ts` — API routes: GET (4 endpoints) (~1361 tok)
- `db.server.test.ts` — Declares db (~560 tok)
- `db.server.ts` — Exports DocumentStatus, UserRecord, DocumentRecord, createDb + 8 more (~1382 tok)
...
- `admin-documents-list.tsx` — loader (~381 tok)
- `admin-upload.tsx` — MAX_UPLOAD_BYTES — renders form (~1012 tok)
- `auth-callback.tsx` — loader (~158 tok)
- `document-page-image.tsx` — loader (~316 tok)
- `document-viewer.tsx` — loader (~873 tok)
- `documents-list.tsx` — loader (~479 tok)
- `home.tsx` — loader (~103 tok)
- `login.tsx` — loader (~113 tok)
- `logout.tsx` — loader — renders form (~237 tok)
